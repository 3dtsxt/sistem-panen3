...
Sistem sederhana untuk mencatat dan menghitung total hasil panen dari beberapa lahan/komoditas.
"""
 
 
def input_data_panen():
    """Meminta pengguna memasukkan data hasil panen."""
    data_panen = []
    print("=== Input Data Hasil Panen ===")
    print("(Ketik 'selesai' pada nama komoditas untuk mengakhiri input)\n")
 
    while True:
        nama = input("Nama komoditas/lahan: ").strip()
        if nama.lower() == "selesai":
            break
 
        try:
            jumlah = float(input(f"Jumlah hasil panen '{nama}' (kg): "))
            if jumlah < 0:
        ...
        data_panen.append({"nama": nama, "jumlah": jumlah})
        print(f"Data '{nama}' sebesar {jumlah} kg berhasil ditambahkan.\n")
 
    return data_panen
 
 
def hitung_total(data_panen):
...
def tampilkan_ringkasan(data_panen):
    ...
    if not data_panen:
        print("\nTidak ada data hasil panen yang tercatat.")
        return
 
    print("\n=== Ringkasan Hasil Panen ===")
    print(f"{'No':<4}{'Komoditas/Lahan':<25}{'Jumlah (kg)':>12}")
    print("-" * 41)
 
    for i, item in enumerate(data_panen, start=1):
        print(f"{i:<4}{item['nama']:<25}{item['jumlah']:>12.2f}")
...
def main():
    ...
    tampilkan_ringkasan(data_panen)
